// Вставка скрипта game.js напрямую в DOM
function injectScriptWithSettings(file, settings, node) {
    const th = document.getElementsByTagName(node)[0];
    const s = document.createElement('script');
    s.setAttribute('type', 'text/javascript');
    
    // Передаем настройки через глобальную переменную window.GAME_SETTINGS
    s.innerHTML = `window.GAME_SETTINGS = ${JSON.stringify(settings)};`;
    th.appendChild(s);

    // Вставляем основной скрипт game.js
...
chrome.storage.sync.get(['flowerProbability', 'bombProbability', 'iceProbability', 'isScriptEnabled'], function(settings) {
    const loadedSettings = {
        bombProbability: (settings.bombProbability || 10) / 100,
        flowerProbability: (settings.flowerProbability || 80) / 100,
